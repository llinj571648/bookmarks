...
async function loadData() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const text = decodeURIComponent(escape(atob(result.content)));
      const match = text.match(/window\.data\s*=\s*(\{[\s\S]*\}\s*\});?/);
      if (match) {
        try {
...
function showToast(message) {
  ...
  toast.style.cssText = `
    ...
    bottom: 20px;
    right: 20px;
    background: ${message.includes('请') || message.includes('无效') ? '#f97316' : message.includes('失败') ? '#ef4444' : '#22c55e'};
    color: white;
    padding: 12px 20px;
    border-radius: 8px;
    z-index: 10000;
...
function renderSidebar() {
  ...
  Object.keys(window.data || {}).forEach(category => {
    ...
    div.innerHTML = `<span>${escapeHtml(category)}</span>`;
    div.onclick = () => {
...
function renderCategory(category) {
  ...
  content.querySelector("#site-submit").onclick = async () => {
